...
model = mujoco.Physics.from_xml_path('assets/piper_ik.xml')
# Number of runs
num_runs = 10
...
for run in range(num_runs):
    ...
    rand_vec = np.random.rand(3) * 0.1
    target_position =np.array([0.2 + rand_vec[0],rand_vec[1], 0.1 + rand_vec[2]])
    perturbed = np.array([0.369-rand_vec[0]-0.2 ,0.5 -rand_vec[1],rand_vec[2]+0.1 ])
   
    print(f"Run {run+1}: {perturbed }")
    # target_position = np.array([0.2, 0.2, 0.2])    
    # target_orientation_euler = np.random.rand(3) * np.pi * 0.2
    target_orientation_euler = np.array([180 / 57.2958, 0, -90 / 57.2958])
    ...
    # 设置 mocap 的位置
    model.data.mocap_pos[0] = perturbed
    ...
    model.data.qpos[0:6] = joint_angles[1:7]
